	...
	BEGIN TRAN
		BEGIN TRY
			INSERT INTO [dbo].[Teams]([ClubId], [TeamManagerId], [Name]) 
				VALUES (@ClubId, @TeamManagerId, @Name)
...
BEGIN
	...
	BEGIN TRAN
		BEGIN TRY
			IF (@TeamId IS NULL)
			BEGIN
				RETURN SELECT * FROM [dbo].[Teams]
			END
			ELSE
			BEGIN
				RETURN SELECT TOP 1 * FROM [dbo].[Teams] WHERE [Id] = ISNULL(@TeamId, [Id])
			END
			COMMIT
		END TRY
		BEGIN CATCH
...
BEGIN
	...
	BEGIN TRAN
		BEGIN TRY
			...
			UPDATE [dbo].[Teams] SET [ClubId] = ISNULL(NULLIF(@ClubId, 0), 0)
									,[TeamManagerId] = ISNULL(NULLIF(@TeamManagerId, 0), 0)
